...
const heuristic_touching = (field: GameField, player: number) => {
  let score = 0
...
const heuristic_connectability = (field: GameField, player: number) => {
  let score = 0
  const playerBlocks = field.log.getBlocksPerPlayer(field)
...
const heuristic_enemy_block = (field: GameField, player: number) => {
  let score = 0
  const playerBlocks = field.log.getBlocksPerPlayer(field)
  const lowestField = field.getLowestFromField()
  const other_player = (player === 1) ? 0 : 1;
  playerBlocks[player].forEach((block) => {
...
const get_best_heuristic_play = (field: GameField, player: number) => {
  return get_best_heuristic_play_for_params(field, player, 0.3, 0.5, 1.0, 20.0)
}

const get_best_heuristic_play_for_params = (field: GameField, player: number, connectability_weight: number, enemy_block_weight: number, touching_weight: number, score_weight: number) => {
  ...
  field.getPossiblePlays().forEach((possible_play) => {
    ...
    heuristic_value += gameScore.getScoreForPlayerAtIndex(player) * score_weight
